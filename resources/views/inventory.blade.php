...
<script>
  ...
  class InventoryTable {
    ...
    loadTableRow(datas) {
      datas.forEach(function(data) {
        $('table.inventory-table tbody').append(`
            <tr data-item-id="" data-variation-id="">
              <td class="d-flex"><img src="${data.image}_tn" width="50px"><div class="ml-1 d-flex flex-column justify-content-between"><div><strong style="font-size:0.8rem">${data.name}</strong></div><div>${data.sku}</div></div></td>
              ...
              <td class="text-center">${money(data.asset_value)}</td>
              <td class="text-center">${data.precentage_asset_value}</td>
            </tr>`);
      });
      $('.inventory-table').on('focus', '[contenteditable]', function() {
        var $this = $(this);
      ...
      $('.inventory-table td').on('change', function() {
        alert(123);
      })
    }
